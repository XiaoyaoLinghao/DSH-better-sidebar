...
describe('SidechainView list shell', () => {
  ...
  it('does not schedule a follow-up timer until the initial transcript read settles', async () => {
    vi.useFakeTimers()
    const setTimeoutSpy = vi.spyOn(globalThis, 'setTimeout')
    let resolveInitial!: (snapshot: Awaited<ReturnType<SidechainHistory['fetchTranscript']>>) => void
    const initial = new Promise<Awaited<ReturnType<SidechainHistory['fetchTranscript']>>>(resolve => { resolveInitial = resolve })
    const fetchTranscript = vi.fn(() => initial)
    const sessionFeed = feed({ current: 'parent', byId: {}, subagentsByParent: { parent: catalog({ entries: [child('slow', 'continuable', 'running')] }) } })
    const mounted = mount(createElement(SidechainView, props(sessionFeed, {
      meta: { version: 1, selectedChildId: 'slow' }, history: { fetchTranscript },
    })))
    expect(fetchTranscript).toHaveBeenCalledTimes(1)
    const schedulerCalls = () => setTimeoutSpy.mock.calls.filter(([, delay]) => delay === 250 || delay === 3000)
    expect(schedulerCalls()).toHaveLength(0)
    await act(async () => { vi.advanceTimersByTime(1000); await Promise.resolve() })
    expect(fetchTranscript).toHaveBeenCalledTimes(1)
    await act(async () => { resolveInitial(transcriptSnapshot()); await initial })
  ...
  it('stops confirmation after eight rejected reads without scheduling a ninth attempt', async () => {
    vi.useFakeTimers()
    const oldSnapshot = transcriptSnapshot([{ kind: 'assistant', seq: 1, text: 'old' }])
    const fetchTranscript = vi.fn()
      .mockResolvedValueOnce(oldSnapshot)
    ...
    const sendPrompt = vi.fn(async () => true)
    const sessionFeed = feed({ current: 'parent', byId: {}, subagentsByParent: { parent: catalog({ entries: [child('reject')] }) } })
    const mounted = mount(createElement(SidechainView, props(sessionFeed, {
      meta: { version: 1, selectedChildId: 'reject' }, history: { fetchTranscript, sendPrompt },
    })))
    await act(async () => { await Promise.resolve(); await Promise.resolve() })
    const confirmationStart = fetchTranscript.mock.calls.length
    const input = mounted.container.querySelector<HTMLInputElement>('[data-sidechain-composer-input]')!
    ...
    await act(async () => { await Promise.resolve(); await Promise.resolve() })
    await act(async () => { await vi.advanceTimersByTimeAsync(5000) })
    expect(fetchTranscript.mock.calls.length - confirmationStart).toBe(8)
    expect(mounted.container.querySelector('[data-sidechain-transcript-error]')).not.toBeNull()
    const bounded = fetchTranscript.mock.calls.length
    await act(async () => { await vi.advanceTimersByTimeAsync(5000) })
    expect(fetchTranscript.mock.calls.length).toBe(bounded)
